        ...
        foreach json passenger in passengers {
            if passenger is map<json> {
                ...
                if passenger.hasKey("passengerId") && passenger.hasKey("cost") {
                    ...
                    if costJson is decimal {
                        totalEarnings += costJson;
                    } else if costJson is int {
                        totalEarnings += <decimal>costJson;
                    } else if costJson is float {
                        totalEarnings += <decimal>costJson;
                    } else {
                        log:printWarn("Invalid cost type for passenger: " + passenger.toString());
                    }
                }
            }
        }
